...
private void searchByType() {
    ...
    if (!results.isEmpty()) {
        for (RAMmodel ram : results) {
            System.out.println("Found RAM: " + ram);
        }
    } else {
...
//--------------------delete-----------------------------------------------
  public void deleteRam() {
        ...
        String code = Inputter.inputNonBlankStr("Enter RAM code to delete: "); // Nhập mã RAM

        int result = RamRepo.delete(code); // Gọi phương thức delete từ RAMrepository
        if (result == 1) {
            System.out.println("RAM module with code " + code + " has been marked as inactive.");
        } else {
  ...
  public void printActiveRam() {
        ...
        for (RAMmodel ram : RamRepo.read().values()) { // Duyệt qua tất cả các module RAM
            if (ram.isActive()) { // Kiểm tra nếu module RAM đang hoạt động
                System.out.println(ram); // In ra thông tin của module RAM
            }
        }
    }
}
